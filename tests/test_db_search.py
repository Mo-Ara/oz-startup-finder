...
def test_db_search_respects_limit():
    result = db_search("a", limit=10, db_path=str(DB_PATH))
    assert len(result["results"]) <= 10


def test_db_search_respects_max_limit_fifty():
    result = db_search("a", limit=1_000, db_path=str(DB_PATH))
...
def test_db_get_company_existing():
    result = db_get_company("Acme AI", db_path=str(DB_PATH))
    assert result is not None
    assert result.get("found") is True
    assert result.get("company_name") == "Acme AI"


def test_db_get_company_missing():
    result = db_get_company("NoSuchCo", db_path=str(DB_PATH))
    assert result is not None
    assert result.get("found") is False
    assert result.get("name") == "NoSuchCo"
